...
/*----------------------------------------------------------------------
|   V8MediaServerDelegate::V8MediaServerDelegate
+---------------------------------------------------------------------*/
V8MediaServerDelegate::V8MediaServerDelegate(const char* url_root, 
...
/*----------------------------------------------------------------------
|   V8MediaServerDelegate::ProcessFileRequest
+---------------------------------------------------------------------*/
NPT_Result 
V8MediaServerDelegate::ProcessFileRequest(NPT_HttpRequest&              request, 
                                                const NPT_HttpRequestContext& context,
                                                NPT_HttpResponse&             response)
{
    NPT_HttpUrlQuery query(request.GetUrl().GetQuery());
    
    PLT_LOG_HTTP_MESSAGE(NPT_LOG_LEVEL_FINE, "V8MediaServerDelegate::ProcessFileRequest:", &request);
...
/*----------------------------------------------------------------------
|   V8MediaServerDelegate::ServeFile
+---------------------------------------------------------------------*/
NPT_Result 
V8MediaServerDelegate::ServeFile(const NPT_HttpRequest&        request, 
                                       const NPT_HttpRequestContext& context,
                                       NPT_HttpResponse&             response,
...
/*----------------------------------------------------------------------
|   V8MediaServerDelegate::OnBrowseMetadata
+---------------------------------------------------------------------*/
NPT_Result
V8MediaServerDelegate::OnBrowseMetadata(PLT_ActionReference&          action, 
                                              ...
                                              const PLT_HttpRequestContext& context)
{
    ...
    if (NPT_FAILED(GetFilePath(object_id, filepath))) {
        /* error */
        NPT_LOG_WARNING("V8MediaServerDelegate::OnBrowse - ObjectID not found.");
...
/*----------------------------------------------------------------------
|   V8MediaServerDelegate::OnBrowseDirectChildren
+---------------------------------------------------------------------*/
NPT_Result
V8MediaServerDelegate::OnBrowseDirectChildren(PLT_ActionReference&          action, 
                                                    const char*                   object_id, 
                                                    const char*                   filter,
                                                    NPT_UInt32                    starting_index,
                                                    NPT_UInt32                    requested_count,
                                                    ...
                                                    const PLT_HttpRequestContext& context)
{
    ...
    if (!m_UseCache || NPT_FAILED(m_DirCache.Get(uuid, dir, entries, &cached_entries_time)) || cached_entries_time < info.m_ModificationTime) {
        ...
        if (NPT_FAILED(res)) {
            NPT_LOG_WARNING_1("V8MediaServerDelegate::OnBrowseDirectChildren - failed to open dir %s", (const char*) dir);
        ...
        if (NPT_FAILED(res)) {
            NPT_LOG_WARNING_1("V8MediaServerDelegate::OnBrowseDirectChildren - failed to open sort dir %s", (const char*) dir);
            NPT_CHECK_WARNING(res);
        }
...
/*----------------------------------------------------------------------
|   V8MediaServerDelegate::OnSearchContainer
+---------------------------------------------------------------------*/
NPT_Result
V8MediaServerDelegate::OnSearchContainer(PLT_ActionReference&          action, 
                                               const char*                   object_id, 
...
/*----------------------------------------------------------------------
|   V8MediaServerDelegate::GetFilePath
+---------------------------------------------------------------------*/
NPT_Result
V8MediaServerDelegate::GetFilePath(const char* object_id, 
                                         NPT_String& filepath) 
{
...
/*----------------------------------------------------------------------
|   V8MediaServerDelegate::BuildSafeResourceUri
+---------------------------------------------------------------------*/
NPT_String
V8MediaServerDelegate::BuildSafeResourceUri(const NPT_HttpUrl& base_uri, 
                                                  const char*        host, 
...
/*----------------------------------------------------------------------
|   V8MediaServerDelegate::ExtractResourcePath
+---------------------------------------------------------------------*/
NPT_Result
V8MediaServerDelegate::ExtractResourcePath(const NPT_HttpUrl& url, 
                                                 NPT_String&        file_path)
{
...
/*----------------------------------------------------------------------
|   V8MediaServerDelegate::BuildResourceUri
+---------------------------------------------------------------------*/
NPT_String
V8MediaServerDelegate::BuildResourceUri(const NPT_HttpUrl& base_uri, 
                                              const char*        host, 
                                              const char*        file_path)
{
...
/*----------------------------------------------------------------------
|   V8MediaServerDelegate::BuildFromFilePath
+---------------------------------------------------------------------*/
PLT_MediaObject*
V8MediaServerDelegate::BuildFromFilePath(const NPT_String&             filepath, 
                                               const PLT_HttpRequestContext& context,
                                               ...
                                               bool                          allip /* = false */)
{
    ...
    if (info.m_Type == NPT_FileInfo::FILE_TYPE_REGULAR) {
        ...
        object->m_Title = NPT_FilePath::BaseName(filepath, keep_extension_in_title) + " wow";
